...
- 수락 기준과 완료 조건 정의
- **구현 완료 후 반드시 Playwright MCP로 테스트 수행 — 테스트 통과 전까지 다음 Task 진행 불가**
- API 연동·비즈니스 로직·Notion DB 쿼리 구현 시 각 단계별 즉시 테스트 수행
...
- 작업 파일의 명세서를 따름
- 기능과 기능성 구현
- **⚠️ 구현 직후 반드시 Playwright MCP로 테스트 수행 — 이 단계는 선택이 아닌 필수**
- API 연동, Notion DB 쿼리, 비즈니스 로직 구현 시 각 구현 단계마다 즉시 테스트
- 각 단계 후 작업 파일 내 단계 진행 상황 업데이트
- 구현 완료 후 Playwright MCP를 사용한 E2E 전체 시나리오 테스트 실행
- 테스트 실패 시 원인 파악 → 수정 → 재테스트 사이클 반복
- 테스트 통과 확인 후에만 다음 단계로 진행
...
> **이 Phase를 완료해야 이후 Phase 진행 가능. 실제 기능 구현은 Phase 3부터 시작.**

- **Task 001: 프로젝트 구조 및 라우팅 설정** - 우선순위
  - Next.js App Router 기반 전체 라우트 구조 생성
  - 모든 주요 페이지의 빈 껍데기(skeleton) 파일 생성
  - 공통 레이아웃·네비게이션·사이드바 골격 구현
  - 환경변수(.env.local) 구조 정의

- **Task 002: 타입 정의 및 Notion DB 스키마 설계**
  - TypeScript 인터페이스 및 타입 정의 파일 생성
  - Notion 데이터베이스 프로퍼티 구조 설계 (실제 연동 제외)
  - API 응답 타입 정의 (`ApiResponse<T>` 등 공통 타입)
...
- **Task 005: Notion 데이터베이스 연동 및 API 개발** - 우선순위
  - Notion API 클라이언트 설정 (`@notionhq/client`)
  - Notion DB 쿼리 함수 구현 (조회·파싱·변환)
  - RESTful API Route 구현 및 더미 데이터 교체
  - **[테스트 필수]** Playwright MCP로 각 API 엔드포인트 응답 검증
  - **[테스트 필수]** Notion DB 쿼리 결과 파싱 정확성 테스트

- **Task 006: 인증 및 권한 시스템 구현**
  - 사용자 인증 시스템 구축
  - 권한 기반 접근 제어 구현
  - 보안 미들웨어 및 세션 관리
  - **[테스트 필수]** Playwright MCP로 인증 플로우 E2E 테스트 수행
  - **[테스트 필수]** 미인증 상태에서 보호 경로 접근 차단 검증

- **Task 006-1: 핵심 기능 통합 테스트**
  - Playwright MCP를 사용한 전체 사용자 플로우 E2E 테스트
  - Notion DB 연동 및 비즈니스 로직 시나리오 검증
  - 에러 핸들링 및 엣지 케이스 테스트 (빈 DB, 권한 오류, 타임아웃 등)
...
- **Phase 1: 애플리케이션 골격 구축** ← **반드시 첫 번째 Phase**
  - 전체 라우트 구조와 빈 페이지(skeleton) 생성
  - 공통 레이아웃·네비게이션·사이드바 골격
  - 기본 타입 정의와 인터페이스 구조
  - Notion DB 프로퍼티 구조 설계 (실제 연동 제외)
  - 환경변수 구조 정의
...
- **Phase 3: 핵심 기능 구현 (구현 후 반드시 Playwright MCP 테스트)**
  - Notion 데이터베이스 연동 및 API 개발
  - 인증/권한 시스템 구현
  - 핵심 비즈니스 로직 구현
  - 더미 데이터를 실제 Notion API 호출로 교체
  - **각 Task 완료 직후 Playwright MCP E2E 테스트 필수 실행**
...
- [ ] API 연동·Notion DB 쿼리·비즈니스 로직 Task마다 Playwright MCP 테스트 항목이 명시되었는가?
- [ ] 각 작업 파일에 "## 테스트 체크리스트" 섹션이 포함되었는가?
- [ ] 테스트 통과 전 다음 단계 진행 불가 조건이 Task에 명시되었는가?
- [ ] 모든 사용자 플로우에 대한 E2E 테스트 시나리오가 정의되었는가?
- [ ] 에러 핸들링·엣지 케이스(빈 응답, 권한 오류, 네트워크 오류) 테스트가 고려되었는가?
- [ ] Phase 3에 통합 테스트 전용 Task가 포함되었는가?
- [ ] Notion DB 쿼리 결과 파싱 정확성 테스트가 포함되었는가?
